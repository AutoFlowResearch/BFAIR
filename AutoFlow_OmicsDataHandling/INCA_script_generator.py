...
def symmetrical_metabolites(atomMappingMetabolite_data_I):
    ...
    for cnt_met, met in atomMappingMetabolite_data_I.iterrows():
        if not pd.isna(met["met_symmetry_atompositions"]):
            ...
            met_positions = [
                int(x)
                for x in prepare_input(met["met_atompositions"], "Curly")
            ]
            met_elements = prepare_input(met["met_elements"], "Curly")
            met["met_symmetry_elements"] = met[
                "met_symmetry_elements"
            ].replace("{}", str(np.nan))
            met_symmetry_elements = prepare_input(
                    ...
                    if (
                        ...
                        if not pd.isna(
                            ...
                            ms_data_norm_stdev_int = [
                                float(x)
                                for x in ms_data["intensity_normalized_stdev"]
                                .strip("}{")
                                .split(",")
                            ]
...
def run_INCA_in_MATLAB(
    INCA_base_directory, script_folder, matlab_script, runner_script
):
    """
