    ...
    def generate_molfile_database(self):
        ...
        for i, met in self.metabolite_data.iterrows():
            ...
            if 'kegg.compound' in met.annotations:
                self.keggIDs = [keggID for keggID in met.annotations['kegg.compound']]
                get_from_keggBool = True
            if 'hmdb' in met.annotations:
                self.hmdbIDnums = ['0' * (7-len(hmdbID[4:])) + hmdbID[4:] for hmdbID in met.annotations['hmdb']]
                get_from_hmdbBool = True
            if 'chebi' in met.annotations:
                self.chebiIDs = [chebiID for chebiID in met.annotations['chebi']]
...
def write_rxn_files(rxn_data):
    ...
    for i, rxn in rxn_data.iterrows():
        ...
        if not all([os.path.isfile(f'metabolites/{met}.mol') for met in metabolites]):
            print(
            ...
            with open(os.path.join(path, rxn_filename), "w") as f:
                ...
                # Write all the other reactions with at least 1 metabolite on each side
                else:
...
def parse_reaction_mappings():
    ...
    """
    if not os.path.isdir('mappedRxns'):
        raise RuntimeError("'mappedRxns' directory not present in current working directory")
        ...
        if rxn not in unmapped_list:  
            ...
            with open(f'mappedRxns/rxnFiles/{rxn}', 'r') as f:
                ...
                for j, line in enumerate(lines):
                    ...
                    # and 15 can occur if we have >100 atoms on one side (cols merge)
                    if len(line.split()) in (15,16) :
                        ...
                        if len(lines[j+1].split()) not in (15,16):
                            # Check if current metabolite is reactant or product
                            if not productBool:
                                # Check if any carbons are present
                                if met_mapping:
                                    c_tracked = ['C' for atom in met_mapping]
                                    pos_tracked = list(range(len(met_mapping)))
                                    mapping_dict['reactants_ids_tracked'].append(met_id)
                                    mapping_dict['reactants_mapping'].append(met_mapping)
                                    mapping_dict['reactants_elements_tracked'].append(c_tracked)
                                    mapping_dict['reactants_positions_tracked'].append(pos_tracked)
                                react_cnt += 1
                                if react_cnt == react_lim: 
                                    productBool = True
                                    
                            # Assign metabolite to products if reached reactant limit
                            else: 
                                if met_mapping:
                                    c_tracked = ['C' for atom in met_mapping]
                                    pos_tracked = list(range(len(met_mapping)))
                                    mapping_dict['products_ids_tracked'].append(met_id)
                                    mapping_dict['products_mapping'].append(met_mapping)
                                    mapping_dict['products_elements_tracked'].append(c_tracked)
                                    mapping_dict['products_positions_tracked'].append(pos_tracked)
                                prod_cnt += 1
                                
                                if prod_cnt == prod_lim: 
                                    react_stoich = ['-1' for met in range(len(mapping_dict['reactants_mapping']))]
                                    prod_stoich = ['1' for met in range(len(mapping_dict['products_mapping']))]
    ...
    list(map(chr, range(48, 58))) + ['_']
    
    # Loop through all reactions
    for i, rxn in mapping_data.iterrows():
        try:
            # Convert number mappings to letters
            carbons_list = [atom for met in rxn['reactants_mapping'] for atom in met]                         
            ...
            if len(carbons_list) > 63:
                print(f'Reaction {rxn["rxn_id"]} contains more than 63 carbon atoms')
            else:
        ...
        stoich_str = '{%s}' % (','.join(rxn['reactants_stoichiometry_tracked']))
...
def parse_metabolite_mappings():
    ...
    for i, met in enumerate(metabolite_list):
        ...
        with open(f'metabolites/{met}', 'r') as f:
            ...
            carbon_count_range = '{%s}' % (','.join([str(x) for x in range(carbon_count)]))
            ...
            if check_symmetry(met):
                carbon_count_range_rev = '{%s}' % (','.join([str(x) for x in range(carbon_count-1,-1,-1)]))
...
def generate_INCA_mapping_input(reaction_df, metabolite_df):
    ...
    metabolite_df.to_csv(path_or_buf = os.path.join(os.getcwd(), f'MappingMetabolites.csv'))    
